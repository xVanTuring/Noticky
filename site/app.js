(function () {
  ...
  var ZH = {
    ...
    "f.float.li4": "常驻最上层（可从菜单栏切换，或绑定全局快捷键）、双击折叠、失焦半透明。",
    ...
    "f.md.title": "实时 Markdown 编辑器",
    "f.md.body": "边打字边内联渲染 Markdown —— 无模式切换、不用双击进编辑态，你编辑的就是你阅读的那一面。写纯文本也一样顺手，没有标记就没有样式。",
    "f.md.li1": "<b>所见即所得</b> —— 标题、粗体、列表、引用，边写边渲染。",
    "f.md.li2": "代码块语法高亮、内联图片、LaTeX 数学公式。",
    "f.md.li3": "<b>任务清单</b> —— <code>- [ ]</code> 复选框，标题栏上带实时完成度环。",
    "f.md.li4": "全局字号（12–24），实时同步到每一个打开的窗口。",

    "g.manager.title": "管理窗口",
    "g.manager.body": "原生侧栏 —— 分组、Ungrouped、Archive、Trash。拖便签进分组、全文搜索、右键重命名/移动，以及多选批量操作。",

    "g.tasks.title": "全部任务，一处汇总",
    "g.tasks.body": "内置视图把所有笔记里未勾选的任务汇总到一起，按笔记分组。勾掉一项就直接回写源笔记 —— 打开的窗口实时同步。子任务还没完成时，已完成的父任务仍会保留，嵌套待办不丢上下文。",
