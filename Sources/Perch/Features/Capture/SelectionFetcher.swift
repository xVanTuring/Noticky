...
///
/// 重要前提:
/// - 用户必须在「系统设置 → 隐私与安全性 → 辅助功能」中授权 Perch。
/// - **Perch 必须不在 sandbox 中运行**。沙盒在 IPC 层拦截跨进程 AX 查询
///   (-25204 kAXErrorCannotComplete),即使 TCC 已授权也无效 —— 详见
///   Resources/Perch.entitlements 注释。这是把 sandbox 关掉的核心原因。
/// - 必须在自家窗口抢焦点 *之前* 读 —— 一旦 Perch 成为 frontmost,
///   `frontmostApplication` 就是 Perch 自己,读到的就是 capture 输入框。
...
enum SelectionFetcher {
    ...
    /// 引导用户授权:先把 Perch 注册进 TCC,再深链到辅助功能面板。
    ///
    ...
    ///
    /// **依赖前提:Perch 已关闭 sandbox**。沙盒会在 IPC 层拦截所有跨进程
    /// AX 查询(返回 -25204 kAXErrorCannotComplete),即使 TCC 已授权也无效。
    /// 详见 Resources/Perch.entitlements 注释。
